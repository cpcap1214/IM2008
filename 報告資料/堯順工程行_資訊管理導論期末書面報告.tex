...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{3.2cm}Y}
    ...
    商業模式畫布 & 第 3 章以商業模式畫布的觀點說明本系統如何創造、傳遞與獲取價值。 \\
...
本章以 Business Model Canvas 描述本專案的輕量化 ERP 與 LINE Bot 整合服務如何創造、傳遞與獲取價值。畫布的重點不是取代大型 ERP，而是針對堯順工程行及相似小型工程行的日常流程，提供能快速導入、低學習成本且可持續擴充的管理工具。

\begin{longtable}{L{3.2cm}L{10.5cm}}
  \caption{堯順工程行智慧管理系統商業模式整理} \\
  \toprule
  區塊 & 說明 \\
  \midrule
  \endfirsthead
  ...
  \endhead
  關鍵夥伴 &
  本專案以 LINE 官方帳號與 Bot 平台作為主要互動介面，並以 Firebase 與 Google Cloud 提供雲端資料儲存與系統運作環境；若需進一步提供營運分析，則可結合 Gemini AI 或本地規則引擎作為輔助工具。此外，堯順工程行老闆與現場人員提供真實流程、使用情境與需求回饋，而系統開發、導入與維護團隊則負責將需求轉化為可落地的資訊系統。 \\
  關鍵活動 &
  本系統的核心活動包括維護訂單、商品與客戶資料，整合 LINE 訊息、下單與付款回報流程，並持續追蹤出貨、收款與待確認狀態。系統同時會產生匯出報表與 AI 營運建議，協助業主掌握整體營運情況，並依據實際使用回饋調整欄位與流程設計。 \\
  關鍵資源 &
  支撐本系統運作的主要資源包括 Firestore 即時資料庫、Web 儀表板、LINE Bot，以及累積下來的訂單、出貨、收款與客戶歷史資料。更重要的是，系統建立在對小型工程行日常作業流程的理解之上，因此能比一般大型 ERP 更貼近現場需求。 \\
  價值主張 &
  本專案的價值主張在於，為工程行提供一套客製化且輕量的 ERP 工具，使業主不必導入過於龐大複雜的系統，也能快速完成訂單紀錄、出貨追蹤與帳款管理。透過 LINE 自動提醒與回報機制，系統可降低人工追帳成本，並讓業主即時掌握待出貨、待收款與待確認事項；同時支援 Excel、CSV 與 PDF 匯出，以符合既有使用習慣。 \\
  顧客關係 &
  在顧客關係設計上，本系統強調低學習成本與即時互動。客戶可透過 LINE 接收付款提醒並進行回報，老闆則可在確認後更新帳務狀態，減少雙方資訊落差。整體操作方式盡量貼近日常習慣，使使用者不需要大幅改變既有流程即可逐步導入。 \\
  通路 &
  本系統的主要通路包含接案網站、LINE Bot 與口碑推薦。其中網站負責展示系統功能與導入說明，LINE Bot 作為日常互動與通知的主要入口，而口碑推薦則有助於在同類型工程行之間擴散，形成後續導入機會。 \\
  顧客區隔 &
  本專案主要面向小型工程行，以及那些認為大型 ERP 太重、太複雜、導入成本過高的店家。這些業者通常具有接單分散、出貨跨日、付款方式多元、帳款追蹤不易等特徵，因此更需要一套能快速落地的管理工具。 \\
  成本結構 &
  本系統的主要成本包括 Firebase 與雲端資料庫部署費用、LINE Bot 訊息推播與串接成本，以及 AI API、規則引擎與資料分析維護等技術支出。若擴大導入規模，後續也需考慮教育訓練、資料整理與維護服務的人力成本。 \\
  收入來源 &
  收入來源可由專案導入與客製化建置費、月租型 SaaS 使用費或維護合約構成。若未來成功擴展到多家工程行，則有機會進一步發展為可持續訂閱的服務模式，並延伸出資料分析或進階功能加值服務。 \\
  \bottomrule
\end{longtable}

整體而言，這套商業模式的核心，不是以功能完整性與大型企業流程控制為目標，而是以「夠用、好懂、能立即改善日常管理」為優先。它把小型工程行原本依賴 LINE、口頭與人工記憶的資訊流，轉換成可追蹤、可查詢、可匯出且可分析的營運資料，並以較低的導入門檻換取實際可見的管理效益。
